...
/*
With this executable run, execute following command
...
int main(void) {
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1)
        fatal("set SO_REUSEADDR option to socket");
    ...
    while(1) {
        ...
        if (new_sockfd == -1)
            fatal("failed to establish a connection");
        printf("established a connection with a server: %s:%d \n",
